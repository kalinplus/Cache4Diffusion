...
def decomposition_FFT(x: torch.Tensor, cutoff_ratio: float = 0.1) -> Tuple[torch.Tensor, torch.Tensor]:
    ...
    B, D, H, W = x_fp32.shape
    x_fp32 = x_fp32.reshape(B, D, H * W)
    freq = torch.fft.fft(x_fp32, dim=2)  # FFT on spatial dimension

    freqs = torch.fft.fftfreq(H * W, d=1.0, device=device)
    ...
    low_fp32  = torch.fft.ifft(low_freq_complex,  dim=2).real
    high_fp32 = torch.fft.ifft(high_freq_complex, dim=2).real

    low  = low_fp32.reshape(B, D, H, W).to(device=device, dtype=orig_dtype)
    high = high_fp32.reshape(B, D, H, W).to(device=device, dtype=orig_dtype)
...
def decomposition_DCT(x: torch.Tensor, cutoff_ratio: float = 0.1) -> Tuple[torch.Tensor, torch.Tensor]:
    ...
    dct_2d = dct.dct_2d(x_fp32, norm='ortho')  # 2D DCT-II
    ...
    low = low_spatial.to(dtype=orig_dtype)
    high = high_spatial.to(dtype=orig_dtype)
