...
# Usage: bash evaluate.sh [--gpu GPU_ID] [--model MODEL]
#
# Models:
#   hunyuan (default)
...
if [ "$MODEL" = "flux" ]; then
    ...
    # baseline — evaluate against itself for absolute metrics (CLIP, ImageReward)
    ...
    if [ -d "$FLUX_OUT/naive_ts" ]; then
        while IFS= read -r leaf_dir; do
            ...
            find "$FLUX_OUT/naive_ts" -name '*.png' -printf '%h\n' | sort -u
        )
    fi
    ...
    if [ -d "$FLUX_OUT/smooth" ]; then
        while IFS= read -r leaf_dir; do
            ...
            find "$FLUX_OUT/smooth" -name '*.png' -printf '%h\n' | sort -u
        )
    fi

else
    # ------------------------------------------------------------------ #
    # HunyuanImage evaluation (original behavior)
    # ------------------------------------------------------------------ #
    ...
    # smooth/exp — auto-discover leaf dirs containing .png files
