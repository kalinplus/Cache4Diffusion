...
class ResnetBlock2D(nn.Module):
    ...
    def forward(self, input_tensor: torch.Tensor, temb: torch.Tensor, cache_dic: Dict[str, Any] = None, current: Dict[str, Any] = None, *args, **kwargs) -> torch.Tensor:
        if len(args) > 0 or kwargs.get("scale", None) is not None:
            deprecation_message = "The `scale` argument is deprecated and will be ignored. Please remove it, as passing it will raise an error in the future. `scale` should directly be passed while calling the underlying pipeline component i.e., via `cross_attention_kwargs`."
            deprecate("scale", "1.0.0", deprecation_message)
        ...
        if current['type'] == 'full':
            ...
            cache_store(cache_dic['cache'][-1][current['module']][current['submodule']][current['subsubmodule']][current['idx']], hidden_states)

        else:

            hidden_states = cache_predict(cache_dic['cache'][-1][current['module']][current['submodule']][current['subsubmodule']][current['idx']], cache_dic['fc_alpha'])
