...
class UNetMidBlock2DCrossAttn(nn.Module):
    ...
    def forward(
        ...
        current['idx'] = 0
        hidden_states = self.resnets[0](hidden_states, temb, cache_dic, current)
    ...
    def forward(
        ...
        if self.downsamplers is not None:
            ...
            if current['type'] == 'full':
                ...
                cache_store(cache_dic['cache'][-1][current['module']][current['submodule']][current['subsubmodule']], hidden_states)

            else:
                hidden_states = cache_predict(cache_dic['cache'][-1][current['module']][current['submodule']][current['subsubmodule']], cache_dic['fc_alpha'])
...
class DownBlock2D(nn.Module):
    ...
    def forward(
        self, hidden_states: torch.Tensor, temb: Optional[torch.Tensor] = None, cache_dic: Dict[str, Any] = None, current: Dict[str, Any] = None, *args, **kwargs
    ) -> Tuple[torch.Tensor, Tuple[torch.Tensor, ...]]:
