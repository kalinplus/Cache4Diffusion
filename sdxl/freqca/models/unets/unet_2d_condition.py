...
class UNet2DConditionModel(
    ...
    def forward(
        ...
        from cache_functions import module_cache_init, derivative_approximation, cache_step, cache_step_merge
        ...
        if current['type'] == 'full':
            ...
            # sample.shape: torch.Size([2, 320, 128, 128])

            module_cache_init(cache_dic=cache_dic, current=current)
            derivative_approximation(cache_dic=cache_dic, current=current, feature=sample)

        else:
            if not cache_dic['use_z_cache'] or not current["merge"]:
                sample = cache_step(cache_dic=cache_dic, current=current)
            else:
                sample = cache_step_merge(cache_dic=cache_dic, current=current)
                if current["update"]:
                    derivative_approximation(cache_dic=cache_dic, current=current, feature=sample)
