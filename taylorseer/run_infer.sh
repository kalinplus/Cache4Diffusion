...
for interval in "${intervals[@]}"; do
    for order in "${max_order[@]}"; do
        ...
        outdir="${base_outdir}/N${interval}O${order}F${first_enhance}"
        ...
        echo "Finished interval=$interval, order=$order. Results saved to $outdir"
        echo "-------------N${interval}O${order}F${first_enhance}-------------"
    done
done
