...
def get_file_path(filename: str | os.PathLike, search_from: str | os.PathLike = "."):
    ...
    """
    for root, _dirs, files in os.walk(search_from):
        for name in files:
            if name == filename:
...
def mllm_output_to_dict(input_string, give_up_parsing=False):
    ...
    if input_string.count(delimiter) == 2:
        ...
        if start_index == -1 or end_index == 0:
            ...
            if give_up_parsing:  # if we want to give up parsing
                ...
                json_content = {
                    "score": scores,
                    "reasoning": "System: output is simply a list of scores",
                }
                json_str = json.dumps(json_content)
                input_string = json_str
                start_index = 0
                ...
                json_content = {
                    "score": scores,
                    "reasoning": "System: output is simply a number",
                }
                json_str = json.dumps(json_content)
                input_string = json_str
                start_index = 0
    ...
    if start_index != -1 and end_index not in (-1, start_index):
