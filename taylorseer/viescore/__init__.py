...
class VIEScore:
    def __init__(self, backbone="gpt4o", task="t2i", key_path=None) -> None:
        ...
        if self.task == "t2i":
            self.SC_prompt = f"{self.context}\n{vie_prompts._prompts_0shot_one_image_gen_rule}\n{vie_prompts._prompts_0shot_t2i_rule_SC}"
            self.PQ_prompt = f"{self.context}\n{vie_prompts._prompts_0shot_rule_PQ}"
        elif self.task == "tie":
            self.SC_prompt = f"{self.context}\n{vie_prompts._prompts_0shot_two_image_edit_rule}\n{vie_prompts._prompts_0shot_tie_rule_SC}"
            self.PQ_prompt = f"{self.context}\n{vie_prompts._prompts_0shot_rule_PQ}"
        elif self.task == "t2v":
            self.SC_prompt = f"{self.context}\n{vie_prompts._prompts_0shot_one_video_gen_rule}\n{vie_prompts._prompts_0shot_t2v_rule_SC}"
            self.PQ_prompt = f"{self.context}\n{vie_prompts._prompts_0shot_t2v_rule_PQ}"

    def evaluate(
        self,
        image_prompts,
        text_prompt,
        extract_overall_score_only=False,
