...
class UNet2DConditionModel(
    ...
    def forward(
        ...
        current['module'] = 'downblocks'
        down_block_res_samples = (sample,)
        for i, downsample_block in enumerate(self.down_blocks):
            current['submodule'] = f'{downsample_block.__class__.__name__}_{i}'
            if hasattr(downsample_block, "has_cross_attention") and downsample_block.has_cross_attention:
                ...
                sample, res_samples = downsample_block(hidden_states=sample, temb=emb, cache_dic=cache_dic, current=current)
                if is_adapter and len(down_intrablock_additional_residuals) > 0:
        ...
        if self.mid_block is not None:
            current['module'] = 'midblock'
            current['submodule'] = f'{self.mid_block.__class__.__name__}'
            if hasattr(self.mid_block, "has_cross_attention") and self.mid_block.has_cross_attention:
                ...
                sample = self.mid_block(sample, emb, cache_dic=cache_dic, current=current)
        ...
        # 5. up
        current['module'] = 'upblocks'
        for i, upsample_block in enumerate(self.up_blocks):
            current['submodule'] = f'{upsample_block.__class__.__name__}_{i}'
