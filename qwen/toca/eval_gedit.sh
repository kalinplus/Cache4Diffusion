...
# GEdit 评测脚本 (Qwen-Image-Edit + ToCa)
# 用于评测 ToCa 生成的 GEdit 图片质量
...
for label in "${CONFIG_LIST[@]}"; do
    SAVE_DIR="/home/hkl/Cache4Diffusion/samples/GEdit/qwen-image-edit/toca/${label}"

    # 检查 fullset 是否存在且有图片
    if [ ! -d "${SAVE_DIR}/fullset" ]; then
        echo "=========================================="
        echo "Skipping (no fullset): ${label}"
        echo "=========================================="
        continue
    fi
    ...
    python /home/hkl/Cache4Diffusion/qwen/toca/evaluate_gedit.py \
        --save_dir "${SAVE_DIR}" \
...
# 参数说明：
# --save_dir: 评测图片目录（格式为 samples/GEdit/qwen-image-edit/toca/{label}）
