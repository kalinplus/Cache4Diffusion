...
def apply_teacache_lite_hyimage_pipeline(pipeline, rel_l1_thresh=0.6):
    """
    Apply TeaCache Lite to HunyuanImage pipeline.
    """
    def _denoise_step_teacache_lite(self, latents, timesteps, text_emb, text_mask, byt5_emb, byt5_mask, guidance_scale: float = 1.0, timesteps_r=None, cache_dic=None, current=None):
        """
    ...
    @torch.no_grad()
    def __call_teacache_lite_pipeline(
        self,
        prompt: str,
        shift: int = 5,
        negative_prompt: str = "",
        ...
        **kwargs
        ...
        print("=" * 60)
        print("HunyuanImage Generation Task (TeaCache-Lite)")
        print("-" * 60)
        ...
        for i, t in enumerate(tqdm(timesteps, desc="Denoising", total=len(timesteps))):
            ...
            if self.cfg_distilled:
                noise_pred = self._denoise_step_teacache_lite(
    ...
    pipeline._denoise_step_teacache_lite = MethodType(_denoise_step_teacache_lite, pipeline)
    ...
    pipeline.__class__.__call__ = __call_teacache_lite_pipeline

    loguru.logger.info(f"TeaCache Lite HyImage pipeline applied (rel_l1_thresh={rel_l1_thresh})")
