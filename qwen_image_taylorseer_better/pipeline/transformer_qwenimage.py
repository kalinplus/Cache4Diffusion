...
class QwenImageTransformerBlock(nn.Module):
    ...
    def forward(
        ...
        from cache_functions.cache_utils import shift_cache_history, derivative_approximation_with_smoothing

        if current['type'] == 'full':
            # if cache_dic.get('use_smoothing', False):
            #     print(f"[TS-SMOOTH] Step {current['step']}: Smoothing ENABLED, method={cache_dic.get('smoothing_method', 'exponential')}, alpha={cache_dic.get('smoothing_alpha', 0.9)}")
            ...
            if cache_dic.get('use_smoothing', False):
                ...
                derivative_approximation_with_smoothing(
                    ...
                    alpha=cache_dic.get('smoothing_alpha', 0.9)
                )
            else:
            ...
            # print("[INFO] Taylor approximation forward pass in QwenImageTransformerBlock.")
            # Get modulation parameters for both streams
            img_mod_params = self.img_mod(temb)  # [B, 6*dim]
            txt_mod_params = self.txt_mod(temb)  # [B, 6*dim]
        ...
        #     current['module'] = 'txt'
        #     module_cache_init(cache_dic=cache_dic, current=current)
        #     derivative_approximation(cache_dic=cache_dic, current=current, feature=encoder_hidden_states)
