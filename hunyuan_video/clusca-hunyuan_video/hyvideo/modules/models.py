...
from .cache_functions import cal_type, force_init, cache_cutfresh, update_cache, propagation_update_cache, cache_cutfresh_img_and_txt
...
class MMDoubleStreamBlock(nn.Module):
    ...
    def forward(
        ...
        if current['type'] == 'full':
            ...
            force_init(cache_dic, current, img)
            ...
            if current['layer'] == 19 and cache_dic['mode'] == 'ClusCa' and cache_dic['cluster_num']:
                # print(f"!!! ClusCa: {current['step']}")
            ...
            force_init(cache_dic, current, txt)
            taylor_cache_init(cache_dic, current)
            ...
            fresh_indices, fresh_tokens_img = cache_cutfresh_img_and_txt(cache_dic=cache_dic, tokens=img, current=current)
            ...
            if cache_dic['propagation_ratio'] > 0.0:
                propagation_update_cache(fresh_indices, fresh_tokens=fresh_tokens_img, cache_dic=cache_dic, current=current)
            else:
            ...
            fresh_indices, fresh_tokens_txt = cache_cutfresh_img_and_txt(cache_dic=cache_dic, tokens=txt, current=current)
...
class MMSingleStreamBlock(nn.Module):
    ...
    def load_mlp_in_weights(self):
        """
        将原来的 linear1 层的权重拆分加载，仅保留 mlp 隐藏层部分。

        参数:
          - linear1_weight: 张量，形状为 (hidden_size * 3 + mlp_hidden_dim, hidden_size)
          - linear1_bias: 张量，形状为 (hidden_size * 3 + mlp_hidden_dim,) 或 None
        """
        # mlp_in
        self.mlp_in = nn.Linear(self.hidden_size, self.mlp_hidden_dim)

        device = self.linear1.weight.device  # 目标设备

        # 直接取出 linear1_weight 的后段，并移动到目标设备
    ...
    def forward(
        ...
        if current['type'] == 'full':
            ...
            #self.load_mlp_in_weights()

            current['module'] = 'mlp'
            fresh_indices, fresh_tokens_mlp = cache_cutfresh(cache_dic=cache_dic, tokens=x, current=current)
            fresh_tokens_mlp = modulate(self.pre_norm(fresh_tokens_mlp), shift=mod_shift, scale=mod_scale)
            #mlp_fresh = self.mlp_in(x_mod)
            _, mlp_fresh = torch.split(
                self.linear1(fresh_tokens_mlp), [3 * self.hidden_size, self.mlp_hidden_dim], dim=-1
            )

            current['module'] = 'attn'
            attn_cache = taylor_formula(cache_dic, current)
...
class HYVideoDiffusionTransformer(ModelMixin, ConfigMixin):
    ...
    def forward(
        ...
        **kwargs, ##
        ...
        # print("tt, th, tw", tt, th, tw) [17, 30, 40]
        ...
        # print(f"{current['step']}:{current['type']}") 
        ...
        if current['compute']:
            ...
            x = cache_dic['aggressive_feature']

        if cache_dic['duca']:
            cache_dic['aggressive_feature'] = x
