...
class Kmeans:
    def __init__(self, n_clusters:int, init:str ='random', max_iters=10, p=1, T_wise=False):
    ...
    def fit(self, X, cache_centroids=None):
        B, N, dim = X.shape
        if self.T_wise:
            X = X.reshape(B*17, N//17, dim)
        if cache_centroids is not None:
            centroids = cache_centroids
        else:
            centroids = self.init_centroids(X) # [1, 17, 4, 3072]
        for _ in range(self.max_iters):
            X_float = X.to(torch.float32)  # [B, 17, N//17, D]:[1, 17, 1200, 3072]            
            centroids_float = centroids.to(torch.float32)  # 将 centroids 转换为 Float 类型 [B, 17,  K, D]:[1, 17, 4, 3072]
            dists = torch.cdist(X_float, centroids_float, p=self.p)  # [B, 17, N//17, K]:[1, 17, 1200, 4]
            labels = dists.argmin(dim=-1)  # [B, 17, N//17]:[1, 17, 1200]
            one_hot = F.one_hot(labels, num_classes=self.n_clusters).type_as(X)  # [B, 17, N//17, K]:[1, 17, 1200, 4]
            counts = one_hot.sum(dim=1).type_as(X) + 1e-8  # [B, 17, K]:[1, 17, 4]
