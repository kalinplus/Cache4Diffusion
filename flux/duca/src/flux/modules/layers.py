...
class DoubleStreamBlock(nn.Module):
    ...
    def forward(self, img: Tensor, txt: Tensor, vec: Tensor, pe: Tensor, cache_dic: dict, current: dict) -> tuple[Tensor, Tensor]:
        ...
        if current['type'] == 'full':
            ...
            cache_dic['cache'][-1][current['stream']][current['layer']][current['module']] = self.img_attn.proj(img_attn)
            img = img + img_mod1.gate * cache_dic['cache'][-1][current['stream']][current['layer']][current['module']]

            current['module'] = 'img_mlp'
            force_init(cache_dic=cache_dic, current=current, tokens=img)
            cache_dic['cache'][-1][current['stream']][current['layer']][current['module']] = self.img_mlp((1 + img_mod2.scale) * self.img_norm2(img) + img_mod2.shift)
            img = img + img_mod2.gate * cache_dic['cache'][-1][current['stream']][current['layer']][current['module']]

            # calculate the txt blocks
            current['module'] = 'txt_attn'
            cache_dic['cache'][-1][current['stream']][current['layer']][current['module']] = self.txt_attn.proj(txt_attn)
            txt = txt + txt_mod1.gate * cache_dic['cache'][-1][current['stream']][current['layer']][current['module']]

            current['module'] = 'txt_mlp'
            force_init(cache_dic=cache_dic, current=current, tokens=txt)
            cache_dic['cache'][-1][current['stream']][current['layer']][current['module']] = self.txt_mlp((1 + txt_mod2.scale) * self.txt_norm2(txt) + txt_mod2.shift)
            txt = txt + txt_mod2.gate * cache_dic['cache'][-1][current['stream']][current['layer']][current['module']]
...
class SingleStreamBlock(nn.Module):
    ...
    def load_mlp_in_weights(self, linear1_weight: torch.Tensor, linear1_bias: Optional[torch.Tensor] = None):
        ...
        if not hasattr(self, 'mlp_in'):
            self.mlp_in = nn.Linear(hidden_size, self.mlp_hidden_dim)
            self.mlp_in.__flops__ = 0 # type: ignore
            self.mlp_in.__macs__ = 0 # type: ignore
    ...
    def forward(self, x: Tensor, vec: Tensor, pe: Tensor, cache_dic: dict, current: dict) -> Tensor:
        ...
        if current['type'] == 'full':
            ...
            cache_dic['cache'][-1][current['stream']][current['layer']][current['module']] = attn

            # compute activation in mlp stream, cat again and run second linear layer
            current['module'] = 'total'
            output = self.linear2(torch.cat((attn, self.mlp_act(mlp)), 2))
            force_init(cache_dic=cache_dic, current=current, tokens=output)
            cache_dic['cache'][-1][current['stream']][current['layer']][current['module']] = output
            ...
            current['module'] = 'attn'
            fake_fresh_attn = torch.gather(input = cache_dic['cache'][-1][current['stream']][current['layer']][current['module']], dim = 1, 
                                            index = fresh_indices.unsqueeze(-1).expand(-1, -1, cache_dic['cache'][-1][current['stream']][current['layer']][current['module']].shape[-1]))
            ...
            output = cache_dic['cache'][-1][current['stream']][current['layer']][current['module']]        
