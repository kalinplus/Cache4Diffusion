...
from flux.sampling import get_noise, get_schedule, prepare, prepare_kontext, prepare_fill, unpack, denoise
...
def main(opts: SamplingOptions):
    ...
    for batch_idx in range(num_prompt_batches):
        ...
        for image_idx in range(opts.num_images_per_prompt):
            ...
            # Denoising
            with torch.no_grad():
                x = denoise(model, **inp, timesteps=timesteps, guidance=opts.guidance, test_FLOPs=opts.test_FLOPs, monitor_gpu_usage=opts.monitor_gpu_usage)
