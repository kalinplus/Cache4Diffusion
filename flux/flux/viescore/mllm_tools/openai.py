...
def load_image(image: str | Image.Image, format: str = "RGB", size: tuple | None = None) -> Image.Image:
    """
    ...
    def extract_response(self, response):
        ...
        try:
            ...
            if response["error"]["code"] == "content_policy_violation":
                ...
                response["error"]["code"] == "rate_limit_exceeded" or response["error"]["code"] == "insufficient_quota"
            ):
                print(f"Code is {response['error']['code']}")
                print(response["error"]["message"])
                if self.multiple_api_keys:
