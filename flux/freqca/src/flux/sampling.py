...
def prepare_kontext(
    ...
    img_cond_pil: Image.Image | None = None,
    ...
    if img_cond_path is not None:
        img_cond = Image.open(img_cond_path).convert("RGB")
    elif img_cond_pil is not None:
        img_cond = img_cond_pil.convert("RGB")
    else:
        raise ValueError("Either img_cond_path or img_cond_pil must be provided")
...
def denoise_cache(
    ...
    **kwargs,
    ...
    if cache_dic['monitor_gpu_usage']:
        ...
        current_mem_start, peak_mem_start = monitor_gpu_memory()
        print(f"GPU memory at the beginning: {current_mem_start:.4f}GB, peak: {peak_mem_start:.4f}GB")
    ...
    for _ in range(8 * cache_dic['num_steps']):

        from .modules.cache_functions import cal_type
        cal_type(cache_dic=cache_dic, current=current)

        img_input = current['img']
        img_input_ids = img_ids

        t_curr, t_prev = timesteps[current['step']], timesteps[current['step'] + 1]
        t_vec = torch.full((img_input.shape[0],), t_curr, dtype=img_input.dtype, device=img_input.device)
        ...
        if img_input_ids is not None:
            pred = pred[:, : current['img'].shape[1]]

        current['img'] = current['img'] + (t_prev - t_curr) * pred

        current['step'] += 1

        if current['step'] == cache_dic['num_steps']:
            break
