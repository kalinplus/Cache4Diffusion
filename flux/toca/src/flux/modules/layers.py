...
class DoubleStreamBlock(nn.Module):
    ...
    def forward(self, img: Tensor, txt: Tensor, vec: Tensor, pe: Tensor, cache_dic: dict, current: dict) -> tuple[Tensor, Tensor]:
        ...
        if current['type'] == 'full':
            ...
            cache_dic['cache'][-1][current['stream']][current['layer']]['attn'] = attn
            ...
            cache_dic['cache'][-1][current['stream']][current['layer']]['img_mlp'] = self.img_mlp((1 + img_mod2.scale) * self.img_norm2(img) + img_mod2.shift)
            img = img + img_mod2.gate * cache_dic['cache'][-1][current['stream']][current['layer']]['img_mlp']

            current['module'] = 'txt_mlp'
            force_init(cache_dic=cache_dic, current=current, tokens=txt)
            # calculate the txt blocks
            txt = txt + txt_mod1.gate * self.txt_attn.proj(txt_attn)
            cache_dic['cache'][-1][current['stream']][current['layer']]['txt_mlp'] = self.txt_mlp((1 + txt_mod2.scale) * self.txt_norm2(txt) + txt_mod2.shift)
            txt = txt + txt_mod2.gate * cache_dic['cache'][-1][current['stream']][current['layer']]['txt_mlp']
...
class SingleStreamBlock(nn.Module):
    ...
    def forward(self, x: Tensor, vec: Tensor, pe: Tensor, cache_dic: dict, current: dict) -> Tensor:
        ...
        if current['type'] == 'full':
            ...
            cache_dic['cache'][-1][current['stream']][current['layer']]['mlp'] = mlp
            ...
            cache_dic['cache'][-1][current['stream']][current['layer']]['total'] = output
            ...
            fake_fresh_attn = torch.gather(input = cache_dic['cache'][-1][current['stream']][current['layer']]['attn'], dim = 1, 
                                            index = fresh_indices.unsqueeze(-1).expand(-1, -1, cache_dic['cache'][-1][current['stream']][current['layer']]['attn'].shape[-1]))
            ...
            output = cache_dic['cache'][-1][current['stream']][current['layer']]['total']
