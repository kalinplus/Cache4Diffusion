...
from flux.modules.cache_functions import force_init, cache_cutfresh, cache_cutfresh_by_cluster, update_cache, propagation_update_cache

from flux.taylor_utils import taylor_formula, derivative_approximation, taylor_cache_init, taylor_formula_for_compile
    ...
    def forward(self, img: Tensor, txt: Tensor, vec: Tensor, pe: Tensor, **kwargs) -> tuple[Tensor, Tensor]:
        ...
        if cache_dic is None:
            ...
            if (current['type'] == 'full') or (current['type'] == 'Delta-Cache'):    
                ...
                if current['layer'] == 18 and cache_dic['mode'] == 'ClusCa':
                ...
                fresh_tokens_txt = self.txt_mlp((1 + txt_mod2.scale) * self.txt_norm2(fresh_tokens_txt) + txt_mod2.shift)
                # if cache_dic['propagation_ratio'] > 0.0:
                #     propagation_update_cache(fresh_indices, fresh_tokens=fresh_tokens_txt, cache_dic=cache_dic, current=current)
                # else:
...
@torch.compile
def seer_add(x:torch.Tensor, module_dict:dict, distance:int, gate)->torch.Tensor:
    x = x + gate * taylor_formula_for_compile(module_dict = module_dict, distance=distance)
    return x
    ...
    def forward(self, x: Tensor, vec: Tensor, pe: Tensor, **kwargs) -> Tensor:
        ...
        if cache_dic is None:
            ...
            if current['type'] == 'full':
                ...
                # fresh_tokens_output = self.linear2(torch.cat((fake_fresh_attn, self.mlp_act(mlp_fresh)), 2))
                # if cache_dic['propagation_ratio'] > 0.0:
                #     propagation_update_cache(fresh_indices=fresh_indices, fresh_tokens=fresh_tokens_output, cache_dic=cache_dic, current=current)
                # else:
                #     update_cache(fresh_indices=fresh_indices, fresh_tokens=fresh_tokens_output, cache_dic=cache_dic, current=current)
                # output = taylor_formula(cache_dic=cache_dic, current=current)
        ...
        #     if current['type'] == 'full' and current['layer'] == 37 and cache_dic['mode'] == 'ClusCa':
