...
from cache_functions import Attention, cal_type, cache_cutfresh, update_cache, propagation_update_cache, force_init
...
class DiTBlock(nn.Module):
    ...
    def forward(self, x, c, current, cache_dic):
        ...
        if current['type'] == 'full':  # Force Activation: Compute all tokens and save them in cache
        ...
        elif current['type'] == 'ClusCa':  # Partial Computation: Compute only fresh tokens selected from each cluster and save them in cache, no attention token computation in the final version
            
            # AdaLN Modulation
            shift_msa, scale_msa, gate_msa, shift_mlp, scale_mlp, gate_mlp = self.adaLN_modulation(c).chunk(6, dim=1)
            
            # LayerNorm FLOPs (for norm2)
            if test_FLOPs:
                flops += B * N * C
            ...
            # x = x + gate_msa.unsqueeze(1) * taylor_formula(cache_dic, current)
            x = seer_add(x=x,
                         module_dict=cache_dic['cache'][-1][current['layer']][current['module']],
            ...
            if cache_dic['propagation_ratio'] > 0.0:
                propagation_update_cache(fresh_indices, fresh_tokens=fresh_tokens, cache_dic=cache_dic, current=current)
            else:
...
def seer_add(x:torch.Tensor, module_dict:dict, distance:int, gate)->torch.Tensor:
    x = x + gate.unsqueeze(1) * taylor_formula(module_dict = module_dict, distance=distance)
    return x
