...
def load_model(model_path: str, torch_dtype: torch.dtype, device: str):
    ...
    try:
        # 加载DiffusionPipeline
        # 不强制使用 variant，兼容本地无 fp16 分片的权重
...
def apply_memory_efficient_options(pipeline, args):
    """在可用情况下开启内存优化（attention/vae 切片与tiling）。"""
    ...
    try:
        # 一些管线提供在 pipeline 或 pipeline.vae 上的 tiling 接口
...
def generate_image(pipeline, args):
    ...
    # 若启用 TaylorSeer，则将缓存上下文挂到核心模块实例属性，避免修改 pipeline.__call__ 的签名
