...
from cache_functions import cache_cutfresh, propagation_update_cache, update_cache
from cluster_utils import get_cluster_info

def clusca_flux_double_block_forward(
    ...
    if current['type'] == 'full':
        ...
        if current['layer'] == self.config.num_layers - 1 and cache_dic['mode'] == 'ClusCa':
            get_cluster_info(hidden_states, cache_dic, current)
        ...
        if current['layer'] == 18 and cache_dic['mode'] == 'ClusCa':
            get_cluster_info(encoder_hidden_states, cache_dic, current)
        ...
        #------------------------------- ClusCa ------------------------------
        fresh_indices, fresh_tokens = cache_cutfresh(hidden_states, cache_dic, current)
        norm_fresh_tokens = self.norm2(fresh_tokens)
        norm_fresh_tokens = norm_fresh_tokens * (1 + scale_mlp[:, None]) + shift_mlp[:, None]

        if cache_dic['propagation_ratio'] > 0.0:
            propagation_update_cache(fresh_indices, norm_fresh_tokens, cache_dic, current)
        else:
            update_cache(fresh_indices, norm_fresh_tokens, cache_dic, current)
        #----------------------------------------------------------------------
        ...
        #-------------------------------- ClusCa -------------------------------------
        fresh_indices, fresh_tokens = cache_cutfresh(encoder_hidden_states, cache_dic, current)
        norm_fresh_tokens = self.norm2_context(fresh_tokens)
        norm_fresh_tokens = norm_fresh_tokens * (1 + c_scale_mlp[:, None]) + c_shift_mlp[:, None]
